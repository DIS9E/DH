...
STYLE_GUIDE = """
🗒️ 작성 규칙  ───────────────────────────────────────────
• 반드시 **HTML 태그**만 사용(코드블록·백틱 X)
• 원문 길이 100 % 유지 – 요약·삭제 금지
• 톤: ‘헤드라이트’ 뉴스레터처럼 **친근한 대화체 + 질문·감탄** 섞기
• 제목은 45자↓ 한국어 · 기사 맥락에 어울리는 🎯 이모지 1–3개
───────────────────────────────────────────────
...
• 제목은 카피라이터처럼 눈길을 끌어야 하며, 관련 이모지 1–3개를 자연스럽게 넣으세요.
• 톤: 친근한 대화체, 질문·감탄 섞기.
• 코드블록, `소제목 1/2` 같은 템플릿 문구 금지.
• 제목은 전부 한국어·기사 분위기에 맞는 이모지(1–3개) 포함·45자 이내.
• 내용 요약·삭제 금지(원문 길이 100% 유지).
"""
