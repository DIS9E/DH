...
def build_brief(cat: str, headline: str) -> str:
    ...
    if cat == "economic":
        try:
            r = requests.get("https://www.nbrb.by/api/exrates/rates/usd?parammode=2",
                             timeout=10).json()
            s.append(f"• NBRB <a href='https://www.nbrb.by'>USD/BLR</a> "
                     f"{r['Cur_OfficialRate']} ({r['Date'][:10]})")
        except:
            pass
    else:
        try:
...
def korean_title(src: str, ctx: str) -> str:
    if not CYRILLIC.search(src):
        return src
    return chat("다음 제목을 한국어 카피라이터 스타일(45자↓, 이모지 1–3개)로:"
                f"\n«{src}»\n문맥:{ctx[:200]}",
